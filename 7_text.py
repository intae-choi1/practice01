...
enemy_x_pos = screen_width/2 - enemy_width/2 # 화면 가로 절반에 위치하게
...
while running:
    ...
    character_x_pos += to_x * dt # 프레임이 낮아지면 dt가 높아짐
    ...
    # 타이머 넣기
    # 경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # ms니까 1000으로 나눠서 s로
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255)) # 시간, 안티앨리어싱, 글자 색
